...
class AssignmentRegistrationModel
{
    ...
    public function registerUser($username, $phone_no,  $email, $password)
    {
        ...
        if (empty($errors)) {
            ...
            $sql = "INSERT INTO membership (name, phone_no, email, password, verificationCode, verify_status) VALUES ('$username', '$phone_no', '$email', md5('$password'), '$verificationCode', '0')";
    ...
    public function verifyUser($verificationCode,$email)
{
    ...
    if ($result->num_rows > 0) {
        ...
        $sql = "UPDATE membership SET verified='verified' WHERE verificationCode='$verificationCode'";
        if ($this->conn->query($sql) === TRUE) {
            return true;
        } else {
            // die('hii');
